...
#define CALL_IF(NUM_BITS, IS_A_8BIT)                                       \
    ...
    cudaFuncSetAttribute(                                                  \
        nanovllm_marlin::awq_marlin_repack_kernel<nanovllm_marlin::repack_threads, NUM_BITS, \
                                         IS_A_8BIT>,                       \
        cudaFuncAttributeMaxDynamicSharedMemorySize, max_shared_mem);      \
    nanovllm_marlin::awq_marlin_repack_kernel<nanovllm_marlin::repack_threads, NUM_BITS,     \
                                     IS_A_8BIT>                            \
        <<<blocks, nanovllm_marlin::repack_threads, max_shared_mem, stream>>>(      \
...
torch::Tensor awq_marlin_repack(torch::Tensor& b_q_weight, int64_t size_k,
  ...
  TORCH_CHECK(size_k % nanovllm_marlin::tile_k_size == 0, "size_k = ", size_k,
              " is not divisible by tile_k_size = ", nanovllm_marlin::tile_k_size);
  TORCH_CHECK(size_n % nanovllm_marlin::tile_n_size == 0, "size_n = ", size_n,
              " is not divisible by tile_n_size = ", nanovllm_marlin::tile_n_size);
  ...
  torch::Tensor out = torch::empty(
      {size_k / nanovllm_marlin::tile_size, size_n * nanovllm_marlin::tile_size / pack_factor},
      options);
