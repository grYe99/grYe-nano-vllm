...
template <typename scalar_t,  // compute dtype, half or nv_float16
          const nanovllm::ScalarTypeId b_type_id,  // weight MarlinScalarType id
          const nanovllm::ScalarTypeId s_type_id,  // weight scale ScalarType id
...
template <int count, nanovllm::ScalarTypeId type_id>
...
template <nanovllm::ScalarTypeId type_id>
...
template <const nanovllm::ScalarTypeId a_type_id,  // A ScalarType id
          const nanovllm::ScalarTypeId b_type_id,  // B ScalarType id
          const nanovllm::ScalarTypeId c_type_id,  // C ScalarType id
          const nanovllm::ScalarTypeId s_type_id,  // B_SCALE ScalarType id
          const int threads,          // number of threads in a threadblock
          const int thread_m_blocks,  // number of 16x16 blocks in the m
                                      // dimension (batchsize) of the
                                      // threadblock
...
__global__ void Marlin(
  ...
  if constexpr (a_type_id == nanovllm::kFE4M3fn.id()) return;
  #endif

  #if defined(__CUDA_ARCH__) && __CUDA_ARCH__ == 750
  // Turing TensorCore only supports fp16 and int8
  if constexpr (a_type_id != nanovllm::kFloat16.id() && a_type_id != nanovllm::kS8.id())
    return;
  #endif

  #if defined(__CUDA_ARCH__) && __CUDA_ARCH__ == 750
  constexpr auto num_bits = nanovllm::ScalarType::from_id(b_type_id).size_bits();
  // Disable use_fp16_accum for NVFP4 and cases when group_size == -1 &&
  // num_bits == 4
  constexpr bool use_fp16_accum =
      a_type_id == nanovllm::kFloat16.id() &&
      (!(b_type_id == nanovllm::kFE2M1f.id() && s_type_id == nanovllm::kFE4M3fn.id()) &&
  ...
  static constexpr auto a_type = nanovllm::ScalarType::from_id(a_type_id);
  static constexpr auto b_type = nanovllm::ScalarType::from_id(b_type_id);
  static constexpr auto c_type = nanovllm::ScalarType::from_id(c_type_id);
  static constexpr auto s_type = nanovllm::ScalarType::from_id(s_type_id);
  if constexpr (b_type == nanovllm::kFE2M1f) {
    static_assert(s_type == nanovllm::kFE4M3fn && group_blocks == 1 ||
                  s_type == nanovllm::kFE8M0fnu && group_blocks == 2);
  } else if constexpr (s_type == nanovllm::kFE8M0fnu) {
    // MXFP8: FP8 weights with e8m0 microscaling block scales
    static_assert(b_type == nanovllm::kFE4M3fn && group_blocks == 2);
  } else if constexpr (std::is_same<scalar_t, nv_bfloat16>::value) {
    static_assert(s_type == nanovllm::kBFloat16);
  } else if constexpr (std::is_same<scalar_t, half>::value) {
    static_assert(s_type == nanovllm::kFloat16);
  }
  ...
  constexpr bool has_zp = b_type == nanovllm::kU4 || b_type == nanovllm::kU8;
  constexpr bool is_int_type = b_type == nanovllm::kU4 || b_type == nanovllm::kU8 ||
                               b_type == nanovllm::kS4 || b_type == nanovllm::kS8 ||
                               b_type == nanovllm::kU4B8 || b_type == nanovllm::kU8B128;
  // see comments of dequant.h for more details
  constexpr bool dequant_skip_flop =
      is_a_8bit || (b_type == nanovllm::kFE4M3fn && !(s_type == nanovllm::kFE8M0fnu)) ||
      b_type == nanovllm::kFE2M1f && s_type == nanovllm::kFE4M3fn ||
      has_zp && !is_zp_float && !std::is_same<scalar_t, nv_bfloat16>::value ||
      has_zp && !is_zp_float && !(b_type == nanovllm::kU8);

  float global_scale_f32 = 1.0f;

  if constexpr (b_type == nanovllm::kFE2M1f && s_type == nanovllm::kFE4M3fn) {
  ...
  auto matmul = [&](int k, int pipe) {
    ...
    if constexpr (s_type == nanovllm::kFE4M3fn || s_type == nanovllm::kFE8M0fnu) {
  ...
  #pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      if constexpr (b_type_id == nanovllm::kFE2M1f.id()) {
  ...
  #pragma unroll
      ...
      if constexpr (group_blocks != -1) {
        if (group_blocks == 2 || k == 1) {
          if constexpr (a_type == nanovllm::kS8) {
            int2 s_vals[2];
            s_vals[0] = {
  ...
  #pragma unroll
            ...
            float2 s_vals[2];
            if constexpr (s_type_id != nanovllm::kFE8M0fnu.id()) {
