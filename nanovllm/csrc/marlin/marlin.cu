...
torch::Tensor marlin_gemm(
    ...
    nanovllm::ScalarTypeId const& b_type_id, int64_t size_m, int64_t size_n,
...
MarlinFuncPtr get_marlin_kernel(
    const nanovllm::ScalarType a_type, const nanovllm::ScalarType b_type,
    const nanovllm::ScalarType c_type, const nanovllm::ScalarType s_type,
...
exec_config_t determine_exec_config(
    const nanovllm::ScalarType& a_type, const nanovllm::ScalarType& b_type,
    const nanovllm::ScalarType& c_type, const nanovllm::ScalarType& s_type, int prob_m,
...
void marlin_mm(const void* A, const void* B, void* C, void* C_tmp, void* b_bias,
               ...
               int lda, void* workspace, nanovllm::ScalarType const& a_type,
               nanovllm::ScalarType const& b_type, nanovllm::ScalarType const& c_type,
               nanovllm::ScalarType const& s_type, bool has_bias,
  ...
  if (major_capability == 7 && minor_capability == 5) {
    stages = 2;
    TORCH_CHECK(a_type == nanovllm::kFloat16 || a_type == nanovllm::kS8,
                "Turing only support FP16 or INT8 activation.");
  }
  if (a_type == nanovllm::kFE4M3fn) {
    TORCH_CHECK(
...
torch::Tensor marlin_gemm(
    ...
    int64_t size_k, bool is_k_full, bool use_atomic_add, bool use_fp32_reduce,
    bool is_zp_float) {
  nanovllm::ScalarTypeId a_type_id, c_type_id, s_type_id;

  auto c_dtype = a.dtype();
  if (a.scalar_type() == at::ScalarType::Half) {
    a_type_id = nanovllm::kFloat16.id();
    c_type_id = nanovllm::kFloat16.id();
  } else if (a.scalar_type() == at::ScalarType::BFloat16) {
    a_type_id = nanovllm::kBFloat16.id();
    c_type_id = nanovllm::kBFloat16.id();
  } else {
    ...
    if (a.scalar_type() == at::ScalarType::Float8_e4m3fn) {
      a_type_id = nanovllm::kFE4M3fn.id();
    } else if (a.scalar_type() == at::ScalarType::Char) {
      a_type_id = nanovllm::kS8.id();
    } else {
  ...
  s_type_id = c_type_id;
  if (b_type_id == nanovllm::kFE2M1f.id()) {
    if (b_scales.scalar_type() == at::ScalarType::Float8_e4m3fn) {
      s_type_id = nanovllm::kFE4M3fn.id();
    } else if (b_scales.scalar_type() == at::ScalarType::Float8_e8m0fnu) {
      s_type_id = nanovllm::kFE8M0fnu.id();
    } else {
      TORCH_CHECK(false,
                  "When b_type = float4_e2m1f, b_scale scalar type must be",
                  "float8_e4m3fn (for NVFP4) or float8_e8m0fnu (for MXFP4).");
    }
  } else if (b_type_id == nanovllm::kFE4M3fn.id() &&
             b_scales.scalar_type() == at::ScalarType::Float8_e8m0fnu) {
    s_type_id = nanovllm::kFE8M0fnu.id();
  }

  nanovllm::ScalarType a_type = nanovllm::ScalarType::from_id(a_type_id);
  nanovllm::ScalarType b_type = nanovllm::ScalarType::from_id(b_type_id);
  nanovllm::ScalarType c_type = nanovllm::ScalarType::from_id(c_type_id);
  nanovllm::ScalarType s_type = nanovllm::ScalarType::from_id(s_type_id);
  ...
  if (global_scale_or_none.has_value()) {
    ...
    TORCH_CHECK(b_type == nanovllm::kFE2M1f && s_type == nanovllm::kFE4M3fn,
  ...
  if (has_zp) {
    TORCH_CHECK(
        b_type == nanovllm::kU4 || b_type == nanovllm::kU8,
        "b_type must be u4 or u8 when has_zp = True. Got = ", b_type.str());
  } else {
    TORCH_CHECK(b_type == nanovllm::kU4B8 || b_type == nanovllm::kU8B128 ||
                    b_type == nanovllm::kS4 || b_type == nanovllm::kS8 ||
                    b_type == nanovllm::kFE4M3fn || b_type == nanovllm::kFE2M1f,
...
// TORCH_LIBRARY_IMPL moved to torch_bindings_marlin.cpp
// (static initializers may not fire in nvcc-compiled .cu files)
